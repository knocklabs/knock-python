...
class TestBulk:
    ...
    def test_streaming_response_create(self, client: Knock) -> None:
        with client.schedules.bulk.with_streaming_response.create() as response:
...
class TestAsyncBulk:
    ...
    async def test_streaming_response_create(self, async_client: AsyncKnock) -> None:
        async with async_client.schedules.bulk.with_streaming_response.create() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
