...
class TestKnock:
    ...
    def test_base_url_env(self) -> None:
        with update_env(KNOCK_BASE_URL="http://localhost:5000/from/env"):
            client = Knock(bearer_token=bearer_token, _strict_response_validation=True)
    ...
    def test_received_text_for_expected_json(self, respx_mock: MockRouter) -> None:
        ...
        strict_client = Knock(base_url=base_url, bearer_token=bearer_token, _strict_response_validation=True)

        with pytest.raises(APIResponseValidationError):
            strict_client.get("/foo", cast_to=Model)

        client = Knock(base_url=base_url, bearer_token=bearer_token, _strict_response_validation=False)
...
class TestAsyncKnock:
    ...
    def test_base_url_env(self) -> None:
        with update_env(KNOCK_BASE_URL="http://localhost:5000/from/env"):
            client = AsyncKnock(bearer_token=bearer_token, _strict_response_validation=True)
            assert client.base_url == "http://localhost:5000/from/env/"
    ...
    async def test_received_text_for_expected_json(self, respx_mock: MockRouter) -> None:
        ...
        strict_client = AsyncKnock(base_url=base_url, bearer_token=bearer_token, _strict_response_validation=True)

        with pytest.raises(APIResponseValidationError):
            await strict_client.get("/foo", cast_to=Model)

        client = AsyncKnock(base_url=base_url, bearer_token=bearer_token, _strict_response_validation=False)
